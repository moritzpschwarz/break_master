...
edgar_v7 %>% filter(!complete.cases(.)) %>% View()
...
cty_sample[!(cty_sample %in% edgar_v7$country)]
cty_sample[!(cty_sample %in% gdp_stats$country)]
cty_sample[!(cty_sample %in% pop_stats$country)]

edgar_v7 %>% 
  filter(year >= 2000, !complete.cases(.))
...
cty_sample[!(cty_sample %in% full_emissions$country)]

full_emissions %>% 
  filter(year >= 1990, !complete.cases(.))
